...
{
    bool drag_float_control_impl(float& value,
    ...
    bool drag_float2_control_impl(glm::vec2& value,
    ...
    bool drag_float3_control_impl(glm::vec3& values,
    ...
    bool begin_table_impl(StringView text)
    {
        ...
        if(ImGui::BeginTable(text.raw_unsafe(), 2, table_flags))
        {
    ...
    void end_table_impl() { ImGui::EndTable(); }

    bool begin_tree_node_branch_impl(StringView name, uint32 flags, void* id)
    {
        ...
        return ImGui::TreeNodeEx(id, branch_flags | flags,
            "%.*s", static_cast<int>(name.size()), name.raw_unsafe());
    }

    void end_tree_node_branch_impl() { ImGui::TreePop(); }

    bool begin_tree_node_leaf_impl(StringView name, void* id)
    {
        ...
        return ImGui::TreeNodeEx(id, leaf_flags, "%.*s",
            static_cast<int>(name.size()), name.raw_unsafe());
    }

    void end_tree_node_leaf_impl() { ImGui::TreePop(); }

    void vertical_separator(float height)
    {
