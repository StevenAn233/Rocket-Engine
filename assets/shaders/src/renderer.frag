#version 450

#include "../include/binding_points.glsl"

layout(location = 0) out vec4 o_color;
layout(location = 1) out int  o_entity_id;

layout(location = 0) in vec4 v_color;
layout(location = 1) in vec2 v_uv_coord;

layout(location = 2) in flat int v_tex_id;
layout(location = 3) in flat int v_is_tex_grey;
layout(location = 4) in flat int v_entity_id;

layout(binding = Sampler2D_0) uniform sampler2D u_textures[32];

void main()
{
	o_entity_id = v_entity_id;
	vec4 tex_color = texture(u_textures[v_tex_id], v_uv_coord);

	const float alpha_threshold = 0.01; // hard-coded
	if(tex_color.a < alpha_threshold) { discard; }

	if(!bool(v_is_tex_grey))
	{
		o_color = tex_color * v_color;
		return;
	}
	const vec3 make_gray_weights = vec3(0.2126, 0.7152, 0.0722);
	float luminance = dot(tex_color.rgb, make_gray_weights);
	o_color = vec4(vec3(luminance), tex_color.a) * v_color;
}
